...
export default {
  ...
  async created () {
    ...
    let userinfo = await AppBridge.getNativeUserName(this);
    if(id){
      CookieUtil.removeItem("uuid");
      CookieUtil.setItem("uuid",id);
      CookieUtil.setItem("email",userinfo.userName);
      CookieUtil.setItem("phone",userinfo.phone);
      this.appLogin = true;
      this.ifLogin = true;
    }else{
      CookieUtil.removeItem("uuid");
  ...
  methods:{
        ...
        if(!this.ifLogin){
          ...
          try {
            ...
            if(window.WebViewJavascriptBridge){
              this.$bridge.callhandler("trackEvent","Ac_App_1stanniversary_lottery_startbutton");
            }
          }
    ...
    HowToGetChances(){
      try {
        ...
        if(window.WebViewJavascriptBridge){
          this.$bridge.callhandler("trackEvent","Ac_App_1stanniversary_lottery_howtoearn");
        }
      }
      this.whichDialog = 2;
      this.showDialog = true;
    ...
    goDetails(){
      try {
        ...
        if(window.WebViewJavascriptBridge){
          this.$bridge.callhandler("trackEvent","Ac_App_1stanniversary_lottery_T&C");
        }
      }
    ...
    goPrize(){ 
      if(!this.ifLogin){
        ...
        try {
          ...
          if(window.WebViewJavascriptBridge){
            this.$bridge.callhandler("trackEvent","Ac_App_1stanniversary_lottery_myprizes");
          }
        }
    ...
    goToShake(){
      try {
        ...
        if(window.WebViewJavascriptBridge){
          this.$bridge.callhandler('goShakePage',null,(data)=>{});
        }else{
