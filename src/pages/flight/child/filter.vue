...
export default {
	...
	created() {
		this._localAirlines = AirlineManager.loadApiAirlines(this.airlineArr);
		let flightArr = this.$store.getters.flightofall
		...
		if (this.ifIn) {
			if (returnArr != null && returnArr.length != 0) {
				let z = flightArr.length - 1;

				if (flightArr[z].realPrice > flightArr[0].realPrice) {
					flightArrMax = flightArr[z].realPrice + returnArr[z].realPrice;
					flightArrMin = flightArr[0].realPrice + returnArr[0].realPrice;
				} else {
					flightArrMax = flightArr[0].realPrice + returnArr[0].realPrice;
					flightArrMin = flightArr[z].realPrice + returnArr[z].realPrice;
				}
			}
		} else if (!this.ifIn && returnArr != null && returnArr.length != 0) {
