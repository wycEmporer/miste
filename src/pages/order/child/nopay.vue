	...
	<div class="wrapper" id="no_pay">
		<head-top class="head" :operate="returnTripList">
			...
			<template v-for="(item,index) in orders">
				<div slot="title" class="title flex" v-if="index==0" :key="item.id">
					<h2>{{departCity}}</h2>
					<div class="title-img">
						-
					</div>
			...
			<span class="h-cancel" slot="right" v-if="showCancel" @click="cancelOrderFn">Cancel</span>
			<span class="h-cancel" slot="right" v-else></span>
		</head-top>
		<div class="container">
			<div class="content">
    ...
    goBack(){
      ...
      return who;
		...
		isEnter() {
			...
			if(this.orders[0].ticketsinfo[0][0].status == '107'){
