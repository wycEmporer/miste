<template>
  <div class="container">
    ...
    <div :class="{content: showHeaders}">
      <div class="summary">
        <div class="t_title">
          <div class="title font24 clear1 text-left font68 marB10">
            <span class="color111">Order Id:</span>
            <span class="right">{{orderId}}</span>
          </div>
          <div class="title  font24 clear1 font68 text-left marB20">
            <span class="color111">Amount:</span>
            <p class="right lightGreen"><span class="rs ">Rs</span>{{needToPay - useGold | formatDate}}</p>
          </div>
        </div>
        <div class="offers-group" v-if="isLogin && !isDisableUseGold">
          <div class="radio-group-sp align-items-center marB10" @click="radio3">
            <div class="ug flex content-start align-items-center" >
              <div class="round1">
                <div :class="{round2:isSelect}"></div>
              </div>
              <p class="font24 clear1" style="flex:1;">
                <span class="color111">Happy Gold Account Balance</span>
                <span class="right lightGreen">Rs.{{goldBalance | formatDate}}</span>
              </p>
            </div>
          </div>
          <div class="input-group flex content-start align-items-center" v-show="isSelect">
            <input type="number" ref="gold" class="form-control" v-model.number="useGold" @blur="computeGold" @keyup="computeGold" placeholder="Enter Happy Gold Amount Here">
      ...
      <div style="background:#f6f6f6;height:0.64rem;"></div>
      <div class="cont" v-show="!goldPay">
        <h5 class="font6 text-left" style="font-size:0.68rem;padding:0.577rem 0;color:#111;font-weight:bold;">Choose your payment mode</h5>
        <div class="tip_block text-left clear1" v-show="payTip">
          {{payTip}}
          <span class="right" @click="payTip = ''">x</span>
        </div>
        <div class="panel-group" role="tablist" aria-multiselectable="true">
          <div class="panel panel-default" v-if="credit.isShow">
            <div class="panel-heading flex space-between" role="tab" @click.prevent="toggleShow('credit')">
              <h4 class="panel-title">
                <a role="button" aria-expanded="true" aria-controls="collapseOne">
                  <i class="iconfont icon-credit iconfont1" ></i>Credit card
                </a>
              </h4>
              <p class="panel-tag" v-if="credit.content">{{credit.content}}</p>
            </div>
            <collapse-transition>
                    ...
                    <div class="cf-box">
                      <form autocomplete="off">
                        ...
                        <div  class="flex space-between c_group" id="isMonthChange1" style="position:relative;">
                          ...
                          <div class="flex1">
                            <div>
                              <input type="text" class="c_cvv form-control" v-model="credit.cvv" maxlength="4" placeholder="CVV">
                            </div>
          ...
          <div class="panel panel-default" v-if="debit.isShow">
            <div class="panel-heading flex space-between" role="tab" @click="toggleShow('debit')">
              <h4 class="panel-title">
              ...
              <p class="panel-tag" v-if="debit.content">{{debit.content}}</p>
            </div>
                    ...
                    <div>
                      <form autocomplete="off">
                        ...
                        <div class="flex space-between c_group" id="isMonthChange2" style="position:relative;">
                          ...
                          <div class="flex1">
                            <div>
                              <input type="text" class="c_cvv form-control" v-model="debit.cvv" maxlength="4" placeholder="CVV">
                            </div>
          ...
          <div class="panel panel-default" v-if="netBank.isShow">
            <div class="panel-heading flex space-between" role="tab" @click="toggleShow('netBank')">
              <h4 class="panel-title">
                <a class="collapsed" role="button" aria-expanded="false"
                  aria-controls="collapseThree">
                  <i class="iconfont icon-bank"></i>Net banking
                </a>
              </h4>
              <p class="panel-tag" v-if="netBank.content">{{netBank.content}}</p>
            </div>
            <collapse-transition name="collapse">
              <div v-show="netBank.in" role="tabpanel" aria-labelledby="headingThree">
                <div class="panel-body">
                  <div>
                    <div>
                      <form>
                        <div class="bankList c_group" v-if="hotBank.length > 0">
                          <div class="checkError" v-show="netBank.errorShow"> Please choose a bank</div>
                        ...
                        <div class="c_group">
                          <div>
                            <select class="form-control" v-model="netBank.selectId" @change="selectValue">
                              <option value="">Choose your bank</option>
                              <option :value="val.id" v-for="(val, index) in netBank.bankList" :key="index">{{val.bankName}}</option>
                            </select>
                          </div>
                          <p class="tip_word" v-if="netBank.description">{{netBank.description}}</p>
                        </div>
                        <div class="c_group">
          ...
          <div class="panel panel-default" v-if="wallet.isShow">
            <div class="panel-heading flex space-between" role="tab" @click="toggleShow('wallet')">
              <h4 class="panel-title">
                <a class="collapsed" role="button" aria-expanded="false"
              ...
              <p class="panel-tag" v-if="wallet.content">{{wallet.content}}</p>
            </div>
            <collapse-transition name="collapse">
                    ...
                    <div class="cf-box">
                      <form>
                        <div class="bankList c_group">
                          <div class="checkError" v-show="wallet.errorShow">Please choose a mode</div>
                          <div class="flex flexWrap">
                            <div class="hotbank" v-for="(val, index) in wallet.walletList" :key="index">
                              <label>
                                <input type="radio" :value="val.id" v-model="wallet.selectId">
                                <img :src="val.bankpic" class="imgHeight" alt="">
                                <!-- <b class="bankIcon" :class="val.bankName"></b> -->
                              </label>
                            </div>
                          </div>
                          <p class="tip_word" v-if="wallet.description">{{wallet.description}}</p>
                        </div>
                        <div class="c_group">
          ...
          <div class="panel panel-default" v-if="EMI.isShow">
            <div class="panel-heading flex space-between" role="tab" @click="toggleShow('EMI')">
              <h4 class="panel-title">
                <a class="collapsed" role="button" aria-expanded="false"
                  aria-controls="collapseFour">
                  <img class="EMIimg" :src='require("assets/images/payment/EMI.png")' />
                  EMI
                </a>
              </h4>
              <p class="panel-tag" v-if="EMI.content">{{EMI.content}}</p>
            </div>
            <collapse-transition name="collapse">
              <div v-show="EMI.in" role="tabpanel" aria-labelledby="headingFour">
                <div class="panel-body">
                  <div class="credit-form">
                    <div class="cf-box">
                      <form>
                        <!-- <div class="bankList c_group">
                          <div class="checkError" v-show="EMI.errorShow">Please choose a mode</div>
                          <div class="flex flexWrap">
                            <div class="hotbank" v-for="(val, index) in EMI.emiList" :key="index">
                              <label>
                                <input type="radio" :value="val.id" v-model="EMI.selectId">
                                <img :src="val.bankpic" class="imgHeight" alt="">
                                <b class="bankIcon" :class="val.bankName"></b>
                              </label>
                            </div>
                          </div>
                          <p class="tip_word" v-if="EMI.description">{{EMI.description}}</p>
                        </div> -->
                        <div class="c_group">
                          <button type="button" class="btn btn-warning" style="width:100%;" @click="makePayment('EMI')">Pay</button>
                        </div>
                      </form>
                    </div>
                  </div>
          ...
          <div class="panel panel-default" v-if="UPI.isShow">
            <div class="panel-heading flex space-between" role="tab" @click="toggleShow('UPI')">
              <h4 class="panel-title">
                <a class="collapsed" role="button" aria-expanded="false"
              ...
              <p class="panel-tag" v-if="UPI.content">{{UPI.content}}</p>
            </div>
            <collapse-transition name="collapse">
          ...
          <div class="panel panel-default" v-if="paypal.isShow">
            <div class="panel-heading flex space-between" role="tab" @click="toggleShow('paypal')">
              <h4 class="panel-title">
                <a class="collapsed" role="button"  aria-expanded="false"
              ...
              <p class="panel-tag" v-if="paypal.content">{{paypal.content}}</p>
            </div>
            <collapse-transition name="collapse">
      ...
      <div class="bottom-con flex space-between align-items-center">
        <div class="botton-image">
          <img :src="require('assets/images/payment/choose_PCI.png')" alt="">
          <img :src="require('assets/images/payment/choose_GEO.png')" alt="">
        </div>
...
export default {
  ...
  methods: {
    selectValue(){
      // this.netBank.bankList.forEach(e =>{
      //   if(e.id == this.netBank.selectId){
      //     this.netBank.description = e.content;
      //   }
      // })
    },
    parseQueryStr (){
    ...
    toggleShow(type) {
      this.err = false;
      let arr = ["credit", "debit", "wallet", "netBank", "EMI", "UPI", "paypal"];
      if (this[type].in) {
        this[type].in = false;
      } else {
        this[type].in = true;
    ...
    makePayment(payType) {
      ...
      try {
        ...
        if(window.WebViewJavascriptBridge){
          this.$bridge.callhandler('trackEvent','Flight_payment_paypage_paybutton');
        }
      };
    ...
    dataOut(params){
      this.$axios({
        ...
        Toast({
          message: 'status:'+ err.response.status + ' url:'+err.config.url,
          duration:10000
        })
        Indicator.close();
        console.log(err);
...
.font24{font-size: 0.512rem;}
.lightGreen{color:#0A9D78;}
.text-left {
  text-align: left;
}
.offers-group {
...
.tip_block{
  background: rgba(227,164,40, 0.1);
  border-radius: 2px;
  font-size: 0.47rem;
  color: #B57B08;
  line-height: 0.7rem;
...
.c_group:last-child{margin-bottom: 0;}
.btn-warning {
  color: #fff;
...
.bottom-con{
  ...
  .botton-image{
    img{height:0.64rem;}
    img:last-child{ margin:0 0 0 0.64rem; }
  }
  p{
    font-size:0.512rem;
    color: #333333;
