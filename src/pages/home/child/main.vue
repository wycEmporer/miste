<template>
	<div class="wrapper">
        ...
        <div class="flex space-around align-items-center" :class="{tab:isShowBtn}" style="height:60px">
          <div class="tab-item" v-for="tab in tabs" :class="{active: currentTab === tab}" @click="currentTab = tab; emitCustomEvent(tab, 'FlightAndHotel')" :key="tab">
            <div class="img" :class="tab"></div>
            <div class="type fontSz12">{{tab}}</div>
          </div>
        </div>
        <keep-alive>
          <div v-if="currentTab === 'Flights'">
            <div class="trip-control">
              <div class="trip-control-btn flex">
                <div class="btn-wrap flex align-items-center" @click="showOne" :class="{active:isOne}">
                  <i class="radio"></i> <span class="fontSz12 trip-type">One Way</span>
                </div>
                <div class="btn-wrap flex align-items-center" @click="showRound" :class="{active:isRound}">
                  <i class="radio"></i> <span class="fontSz12 trip-type">Round Trip</span>
                </div>
              </div>
            </div>
            <div class="content">
        ...
        <keep-alive>
          <HotelSearch  v-if="currentTab === 'Hotels'"/>
        </keep-alive>
			</div>
			...
			<div class="advBanner">
				<div v-if="showBanner" :class="{'banner':newBottom,'down':oldBottom}">
	...
	async created() {
		...
		User.advList(this, parm2).then(res => {
			if (res.success) {
				...
				}else if(res.list.length > 0 && res.list.length < 2){
	...
	methods: {
      ...
      for(let k in item){
		...
		showRound() {
			this.isOne = false;
			this.isRound = true;
			sessionStorage.setItem("flightType", "two")
...
.tab{
  ...
  .img.Flights{
    background-image: url(../../../assets/images/home/flights-uncheck.png);
  }
  .img.Hotels{
    background-image: url(../../../assets/images/home/hotels-uncheck.png);
  }
  .tab-item{
    box-sizing: border-box;
    padding-top: 12px;
  ...
  .active{
    ...
    .img.Flights{
    background-image: url(../../../assets/images/home/flights-check.png);
    }
    .img.Hotels{
      background-image: url(../../../assets/images/home/hotels-check.png);
    }
    .type{
      color: #333;
    }
